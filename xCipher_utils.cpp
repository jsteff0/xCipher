...
uint8_t ShiftBits(uint8_t byteinput, uint8_t keybyte, int shift, bool inv)
{
	uint8_t newbyte;
	if (!inv)
	{
...
vector<vector<uint8_t> > ShiftColumns(vector<vector<uint8_t> > matrix, bool inv)
{
	if (!inv)
	{
		for (int i = 0; i < 4; i++)
...
uint8_t SubBytes(uint8_t byte, bool inv)
{
	if (!inv)
		return sbox[byte];
	else
...
vector<vector<uint8_t> > MixColumns(vector<vector<uint8_t> > &state, vector<vector<uint8_t> > &miningConstants, vector<vector<uint8_t> > &key, bool inv)
{
...
vector<vector<uint8_t> > SwapBytes(vector<vector<uint8_t> > &state, vector<vector<uint8_t> > &key, bool inv)
{
	if (!inv)
	{
		for (int i = 0; i < 4; i++)
